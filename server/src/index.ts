...
// Serve the benchmark report (generated by `npm run benchmark:dom` in
// extension/) so it can be shown live, offline, with zero dependency on
// claude.ai or any external host — just this same local server.
//
// Deliberately NOT a static mount of the whole benchmark/ directory —
// that folder can contain face-benchmark/images/ (real consented test
// photos) and fixtures.ts source, neither of which should be servable.
// Only the single generated report file is exposed.
app.get("/benchmark/report.html", (_req: Request, res: Response) => {
  const reportPath = path.join(__dirname, "../../extension/benchmark/report.html");
  res.sendFile(reportPath, (err) => {
    if (err) {
      res.status(404).send(
        "Report not generated yet — run `npm run benchmark:dom` in extension/ first."
      );
    }
  });
});
