...
3. **On-device vision** — a small object-detection model (YOLOv10n, via [Transformers.js](https://github.com/xenova/transformers.js) + ONNX Runtime) runs *inside the browser* — WebGPU if available, falling back to WebAssembly (CPU) — to find faces/people in the screenshot. No network call, no cloud, runs on whatever hardware the laptop has.
